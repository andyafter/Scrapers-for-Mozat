...
import constants
from common_utils import getLogger
...
class BaseExtractor(object):
    ...
    def xpath_str(self, response):
        """
        抽取所有命中xpath的部分
        """
        try:
            return ' '.join([x.strip() for x in response.xpath(self.value).extract()])
        except:
            logger.error('valid xpath value:%s' % (self.value,))
            raise

    def xpath_list(self,response):
        """
        抽取所有命中xpath的部分
        """
        ...
        result = []
        try:
            for xpath in self.value:
                result.extend([x.strip() for x in response.xpath(xpath).extract()])
        except:
            logger.error('valid xpath value:%s' % (self.value,))
            raise
        return result

    def xpath_list_str(self, response):
        """
        抽取所有命中xpath的部分, return string
        """
        result = []
        assert type(self.value) == list
        try:

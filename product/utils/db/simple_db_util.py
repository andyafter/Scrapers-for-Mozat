...
def get_db_class_by_type(name):
    ...
    cls = None
    if name == 'mysql':
        from product.utils.db.simple_dbs import MySQL as cls
    else:
...
def test():
    ...
    ### Test fetch_rows()
    rows = conn.fetch_rows(r'''SELECT * FROM test.product p where id >= 1 and id <= 10''')
    assert(len(rows) == 10)
    
    ### Test fetch_row()
    row = conn.fetch_row(r'''SELECT * FROM test.product p where id = 1''')
    assert(row['pid'] == '5017147')
    
    ### Test execute()
    old_price = int(row['price'])
    new_price = old_price+1
    conn.execute(r'''update test.product p set price = price + 1 where id = 1;''')
    new_price_in_db = conn.fetch_number(r'''SELECT price FROM test.product p where id = 1''')
    assert(new_price_in_db == new_price)
    
    print 'End test connection, test is success!!!'
